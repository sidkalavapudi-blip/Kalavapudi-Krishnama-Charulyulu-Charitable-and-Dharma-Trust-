...
import { ShieldCheck, HeartHandshake, Landmark, FileText, CheckCircle, Flame, DollarSign, RefreshCw, Printer, Info, ExternalLink, PhoneCall } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16" id="donate_header">
          ...
          <div className="inline-flex items-center gap-2 bg-emerald-50 border border-emerald-200/80 px-4 py-1.5 rounded-full mb-4 shadow-xs">
            ...
            <span className="text-[10px] text-emerald-950 font-bold uppercase tracking-wider">Trust Donation Portal Certified & Active</span>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="w-full lg:w-7/12 bg-white rounded-2xl border border-amber-100 shadow-xl p-6 sm:p-8 relative overflow-hidden" id="donations_input_wrapper">
            {/* Elegant Under Construction Overlay */}
            {!isSimulatorUnlocked && (
              <div className="absolute inset-0 bg-stone-50/98 backdrop-blur-[4px] z-30 flex flex-col items-center justify-center p-6 text-center">
                <div className="w-14 h-14 rounded-full bg-amber-50 border border-amber-200/60 flex items-center justify-center text-amber-600 mb-4 shadow-xs animate-pulse">
                  <Landmark className="w-7 h-7 text-orange-600" />
                </div>
                <span className="text-stone-500 text-[10px] uppercase font-bold tracking-widest mb-1.5">Awaiting Integration</span>
                <h4 className="font-serif text-lg sm:text-xl font-bold text-amber-950 mb-2 max-w-sm">
                  Online Donation Portal is Under Construction
                </h4>
                <p className="text-stone-600 text-xs sm:text-sm max-w-md mx-auto mb-6 leading-relaxed">
                  We are currently integrating with our bank partners to set up secure UPI, card, and net banking routes. Directly routing to our central treasury will be active soon.
                </p>
                <div className="flex flex-col gap-3 justify-center w-full max-w-sm">
                  <button
                    type="button"
                    onClick={() => setIsSimulatorUnlocked(true)}
                    className="px-5 py-3 bg-gradient-to-r from-orange-600 to-amber-500 hover:from-orange-700 hover:to-amber-600 text-white rounded-xl font-extrabold text-xs uppercase tracking-wider flex items-center justify-center space-x-1.5 shadow-md hover:shadow-lg hover:scale-[1.01] transition-all cursor-pointer"
                  >
                    <HeartHandshake className="w-4 h-4 shrink-0" />
                    <span>Try Donation & 80G Receipt Simulator</span>
                  </button>
                  <a
                    href="tel:+919440123456"
                    className="px-5 py-2.5 bg-stone-150 hover:bg-stone-200 text-stone-700 rounded-xl font-bold text-xs uppercase tracking-wider flex items-center justify-center space-x-1.5 transition-colors cursor-pointer border border-stone-200"
                  >
            ...
            {isSimulatorUnlocked && (
              <div className="mb-6 p-4 bg-orange-50 border border-orange-200/80 rounded-xl flex items-center justify-between gap-4 text-xs text-orange-950 shadow-sm">
                <div className="flex items-center space-x-2.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-orange-500 animate-pulse shrink-0"></span>
                  <span><strong>Simulator Mode Active:</strong> Fill in the form below to preview the secure transaction workflow and instantly generate your printable 80G tax receipt.</span>
                </div>
                <button 
                  onClick={() => setIsSimulatorUnlocked(false)}
                  className="text-[10px] font-bold underline text-amber-950 hover:text-orange-700 shrink-0 uppercase tracking-wider"
                >
                  [Lock Form]
                </button>
              </div>
          ...
          <div className="w-full lg:w-5/12 bg-stone-100 rounded-2xl border border-stone-200/60 p-4 sm:p-6" id="donation_receipt_preview_aside">
            ...
            {completedReceipt ? (
              ...
              <div className="h-[420px] border-2 border-dashed border-stone-300 rounded-xl flex flex-col items-center justify-center p-6 text-center text-stone-400 bg-stone-50/50">
                <FileText className={`w-12 h-12 mb-3 ${isSimulatorUnlocked ? 'text-orange-500 animate-bounce' : 'text-stone-300'}`} />
                <p className="text-sm font-semibold text-stone-600">
                  {isSimulatorUnlocked ? 'Receipt Generator Ready' : 'Gateway Integration Pending'}
                </p>
                <p className="text-xs mt-2 max-w-xs leading-relaxed text-stone-500">
                  {isSimulatorUnlocked 
                    ? "Fill in your donation details in the form on the left and click 'Sponsor' to generate, preview, and print your official 80G Tax Receipt."
                    : "Automated 80G tax receipt generation will be activated once our secure banking nodes are fully connected."}
                </p>
              </div>
            )}
